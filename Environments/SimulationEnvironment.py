...
class SimulationEnvironment:
    def __init__(self, percentage_usage_data, num_interactions_in_kb, alpha, seed):
        ...
        self.train_dataset, self.test_data, self.transactions_in_kb = \
            create_traintest_split_with_knowledge(percentage_usage_data,
                                                num_interactions_in_kb, seed)
        self.index = 0
    ...
    def calculate_map_at_1_sim(self, recommended_arms, policy):
        ...
        average_precision = 0
        if policy == "Random":
            relevance_threshold = self.current_interaction_dataframe.nlargest(1, 'final_ml_value')['final_ml_value'].values[0] - 0.0005
            if self.current_interaction_dataframe[
                self.current_interaction_dataframe['technique_completeness'] == mapping_MethodToArm[recommended_arms[0]]][
                'final_ml_value'].values[0] >= relevance_threshold:
                average_precision += 1
