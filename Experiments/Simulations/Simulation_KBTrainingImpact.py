from Environments.SimulationEnvironment_KBTrainingImpact import SimulationEnvironmentKBImpact
...
########################################################################################################################
init_file_name = "KBImpact"
dt_string = datetime.now().strftime("%Y-%m-%d_%H-%M-%S_NONUSOKB")
...
with open(result_file_path, 'w') as f:
    header = ['ID', 'Experiment', 'Seed', '% Data', '# Train', '# Test', '# Trans_KB',
              'LinUCB_KB CumRew', 'LinUCB_KB CumReg', 'LinUCB_KB MAP@3', 'LinUCB_KB MAP@1'
              ]
    f.write(','.join(header) + '\n')
...
for seed in seeds:
    ...
    for e in range(n_experiments):
        seed = seeds[e]
        print(f'Running experiment {e + 1}/{n_experiments} with {num_interactions_in_kb} transactions in KB ...')

        # Initialize the simulation environment
        env = SimulationEnvironmentKBImpact(percentage_usage_data, num_interactions_in_kb, alpha_reward, seed)
    ...
    ax_cum_reward.plot(range(total_rounds), reward_curr_policy_cum, label=seed)
    ...
    ax_cum_reward.axvline(x=T_train, color='black', linestyle='--',
                      label='End of Training' if seed == seeds[0] else "")
    ...
    ax_cum_regret.plot(range(total_rounds), regret_curr_policy_cum, label=seed)
    ...
    ax_inst_reward.plot(range(total_rounds), mean_cum_reward_curr_policy, label=seed)
    ...
    ax_inst_regret.plot(range(total_rounds), mean_cum_regret_curr_policy, label=seed)
    ...
    ax_accuracy.plot(range(T_test), relevance_curr_policy_cum, label=num_interactions_in_kb)
...
output_file_path = os.path.join(directory, "KBImpact_Img_MostPickedArms.pdf")
