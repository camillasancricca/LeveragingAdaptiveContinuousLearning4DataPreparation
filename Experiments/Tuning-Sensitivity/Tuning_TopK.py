...
# Hyperparameters
########################################################################################################################
topK_params = [1, 2, 3, 5, 10]

########################################################################################################################
# Run the experiments
########################################################################################################################
for topK in topK_params:
    ...
    for e in range(n_experiments):
        seed = seeds[e]
        print(f'Running experiment {e + 1}/{n_experiments} with topK {topK} ...')
    ...
    ax_cum_reward.plot(range(total_rounds), reward_curr_policy_cum, label=topK)
    ...
    ax_cum_reward.axvline(x=T_train, color='black', linestyle='--',
                      label='End of Training' if topK == topK_params[4] else "")
    ...
    ax_cum_regret.plot(range(total_rounds), regret_curr_policy_cum, label=topK)
    ...
    ax_inst_reward.plot(range(total_rounds), mean_cum_reward_curr_policy, label=topK)
    ...
    ax_inst_regret.plot(range(total_rounds), mean_cum_regret_curr_policy, label=topK)
    ...
    ax_accuracy.plot(range(T_test), relevance_curr_policy_cum, label=topK)
