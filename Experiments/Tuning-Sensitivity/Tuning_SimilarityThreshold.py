...
########################################################################################################################
init_file_name = "SimilarityThreshold"
...
init_title = "Similarity Threshold"
...
# Hyperparameters
########################################################################################################################
sim_threshold_params = [0.6, 0.75, 0.85, 0.9, 0.95]

########################################################################################################################
# Run the experiments
########################################################################################################################
for sim_threshold in sim_threshold_params:
    ...
    for e in range(n_experiments):
        seed = seeds[e]
        print(f'Running experiment {e + 1}/{n_experiments} with sim_threshold {sim_threshold} ...')
        ...
        experiment_name = f"SimThreshold_{sim_threshold}"
    ...
    ax_cum_reward.plot(range(total_rounds), reward_curr_policy_cum, label=sim_threshold)
    ...
    ax_cum_reward.axvline(x=T_train, color='black', linestyle='--',
                      label='End of Training' if sim_threshold == sim_threshold_params[4] else "")
    ...
    ax_cum_regret.plot(range(total_rounds), regret_curr_policy_cum, label=sim_threshold)
    ...
    ax_inst_reward.plot(range(total_rounds), mean_cum_reward_curr_policy, label=sim_threshold)
    ...
    ax_inst_regret.plot(range(total_rounds), mean_cum_regret_curr_policy, label=sim_threshold)
    ...
    ax_accuracy.plot(range(T_test), relevance_curr_policy_cum, label=sim_threshold)
