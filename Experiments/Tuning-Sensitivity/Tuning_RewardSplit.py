...
# Hyperparameters
########################################################################################################################
alpha_reward_params = [0.1, 0.25, 0.50, 0.75, 0.9]

########################################################################################################################
# Run the experiments
########################################################################################################################
for alpha_reward in alpha_reward_params:
    ...
    beta_reward = 1 - alpha_reward
    beta_reward = round(beta_reward, 2)

    for e in range(n_experiments):
        seed = seeds[e]
        print(f'Running experiment {e + 1}/{n_experiments} with reward split Alpha={alpha_reward} '
              f'and Beta={beta_reward} ...')
        ...
        experiment_name = f"Alpha_{alpha_reward}_Beta_{beta_reward}"
    ...
    ax_cum_reward.plot(range(total_rounds), reward_curr_policy_cum, label=alpha_reward)
    ...
    ax_cum_reward.axvline(x=T_train, color='black', linestyle='--',
                      label='End of Training' if alpha_reward == alpha_reward_params[4] else "")
    ...
    ax_cum_regret.plot(range(total_rounds), regret_curr_policy_cum, label=alpha_reward)
    ...
    ax_inst_reward.plot(range(total_rounds), mean_cum_reward_curr_policy, label=alpha_reward)
    ...
    ax_inst_regret.plot(range(total_rounds), mean_cum_regret_curr_policy, label=alpha_reward)
    ...
    ax_accuracy.plot(range(T_test), relevance_curr_policy_cum, label=alpha_reward)
