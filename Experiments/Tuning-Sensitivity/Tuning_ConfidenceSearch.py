...
########################################################################################################################
init_file_name = "ConfidenceSearch"
dt_string = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
folder_string = 'Tuning-Sensitivity/' + init_file_name + '/' + dt_string
...
# Hyperparameters
########################################################################################################################
alpha_confidence_params = [0.1, 0.3, 0.5, 0.7, 0.9]

########################################################################################################################
# Run the experiments
########################################################################################################################
for alpha_confidence in alpha_confidence_params:
    ...
    for e in range(n_experiments):
        seed = seeds[e]
        print(f'Running experiment {e + 1}/{n_experiments} with Alpha Confidence: {alpha_confidence} ...')
        ...
        experiment_name = f'AlphaConfidence_{alpha_confidence}'
    ...
    ax_cum_reward.plot(range(total_rounds), reward_curr_policy_cum, label=alpha_confidence)
    ...
    ax_cum_reward.axvline(x=T_train, color='black', linestyle='--',
                      label='End of Training' if alpha_confidence == alpha_confidence_params[4] else "")
    ...
    ax_cum_regret.plot(range(total_rounds), regret_curr_policy_cum, label=alpha_confidence)
    ...
    ax_inst_reward.plot(range(total_rounds), mean_cum_reward_curr_policy, label=alpha_confidence)
    ...
    ax_inst_regret.plot(range(total_rounds), mean_cum_regret_curr_policy, label=alpha_confidence)
    ...
    ax_accuracy.plot(range(T_test), relevance_curr_policy_cum, label=alpha_confidence)
...
output_file_path = os.path.join(directory, "Confidence_Img_MostPickedArms.pdf")
